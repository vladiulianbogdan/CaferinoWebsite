...
# Create your models here.
class AfterSchoolEvent(models.Model):
    ...
    def publish_url(self):
        return "https://cofferinohub.ro/after-school/register/" + str(self.id)
    
    def show_participants(self):
        print("show part")
        participations = AfterSchoolParticipation.objects.filter(event_id=self.id)
        print("nimic")

        string = "<ul>"

        for participation in participations:
            string += "<li><a href='/admin/afterschool/afterschoolparticipation/" + str(participation.id) +  "/change/'> " + participation.name + ", " + participation.email + ", " + participation.phone + ", Nume copil:" + participation.childName + ", Varsta copil " + participation.childAge + "</a></li>"

        string += "</ul>"

        return format_html(string)

class AfterSchoolEventAdmin(admin.ModelAdmin):
...
class AfterSchoolParticipation(models.Model):
    id = models.AutoField(primary_key=True)
    event_id = models.ForeignKey(AfterSchoolEvent, on_delete=models.CASCADE)
